...
describe('quoteSqlWhereIdentifiers', () => {
  ...
  it('quotes field names for contains / LIKE-style filters', () => {
    ...
    expect(quoteSqlWhereIdentifiers(`(host LIKE 'prod%')`)).toBe(`("host" LIKE 'prod%')`);
    expect(quoteSqlWhereIdentifiers(`(host LIKE '%prod')`)).toBe(`("host" LIKE '%prod')`);
  });
  ...
  it('does not rewrite time-filter macros', () => {
    expect(quoteSqlWhereIdentifiers(`$__timeFilter(createdAt)`)).toBe(`$__timeFilter(createdAt)`);
  });
