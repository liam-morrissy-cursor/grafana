...
func TestIntegrationDuplicatesValidator(t *testing.T) {
	...
	t.Run("Duplicates validator should restrict write access only for readers with duplicates", func(t *testing.T) {
		...
		reader3, err := NewDashboardFileReader(cfg3, logger, fakeService, fakeStore, folderSvc, cfgT, nil)
		require.NoError(t, err)
		...
		r, err = NewDashboardFileReader(cfg3, logger, fakeService, fakeStore, folderSvc, cfgT, nil)
		require.NoError(t, err)
