...
func Initialize(ctx context.Context, cfg *setting.Cfg, opts server.Options, apiOpts api.ServerOptions) (*server.Server, error) {
	...
	zanzanaServer, err := authz.ProvideEmbeddedZanzanaServer(cfg, sqlStore, tracingService, featureToggles, registerer, eventualRestConfigProvider, storeProvider, v, elector)
	if err != nil {
		return nil, err
	}
	...
	correlationsCorrelationsService, err := correlations.ProvideLegacyService(sqlStore, routeRegisterImpl, service13, accessControl, quotaService)
	if err != nil {
		return nil, err
	}
	correlationsAppInstaller, err := correlations2.RegisterAppInstaller(cfg, featureToggles, correlationsCorrelationsService)
	if err != nil {
		return nil, err
	}
...
func InitializeForTest(ctx context.Context, t sqlutil.ITestDB, testingT interface {
	Cleanup(func())
	mock.TestingT
}, cfg *setting.Cfg, opts server.Options, apiOpts api.ServerOptions) (*server.TestEnv, error) {
	routeRegisterImpl := routing.ProvideRegister()
	configProvider, err := configprovider.ProvideService(cfg)
	if err != nil {
		return nil, err
...
func InitializeForCLI(ctx context.Context, cfg *setting.Cfg) (server.Runner, error) {
	...
	runner := server.NewRunner(cfg, sqlStore, ossImpl, serviceService, featureToggles, secretsService, secretsMigrator, userimplService, consolidationService, dependencyRegisterer)
	return runner, nil
}
